...
// Hash the public request source for rate limiting.
function fingerprint(kind: PublicRequestKind, requestHeaders: Headers) {
  ...
  if (!secret) {
    throw new Error("Public request protection is not configured.");
  }
        ...
        async (transaction) => {
          ...
          if (!existing) {
            await transaction.publicRequestAttempt.create({
              data: { fingerprint: requestFingerprint, kind, windowStartedAt: now },
            });
            return;
          }
