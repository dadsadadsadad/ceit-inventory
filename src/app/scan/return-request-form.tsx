...
export function ReturnRequestForm({ itemName, qrCode }: { itemName: string; qrCode: string }) {
  return (
    <FeedbackForm action={submitReturnRequest} className="card rounded-lg p-5 sm:p-7">
      ...
      {/* Borrower details used to find the active loan. */}
      ...
      <label className="mt-4 block">
        ...
        <textarea
          ...
          maxLength={1_000}
          className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm"
          placeholder="Mention damage, missing accessories, or anything staff should know."
        />
      </label>
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="muted text-xs leading-5">
          A return is only completed after a CEIT staff member confirms it.
        </p>
        <SubmitButton
          pendingLabel="Sending return request…"
          className="secondary-button w-full rounded-lg px-4 py-2.5 text-sm font-semibold sm:w-auto"
        >
          Request return
        </SubmitButton>
      </div>
