...
import { BarChart3, Boxes, HandHelping, LayoutDashboard, Menu, Package, ScanLine, ScrollText, Settings, Users, Wrench, X } from "lucide-react";
    ...
    <aside className="dashboard-sidebar border-b lg:flex lg:w-72 lg:flex-col lg:self-stretch lg:border-b-0 lg:border-r">
      <div className="px-5 py-5 lg:px-6 lg:py-7">
        <div className="flex items-center gap-3">
          ...
          <button type="button" aria-expanded={menuOpen} aria-controls="dashboard-navigation" aria-label={menuOpen ? "Close navigation" : "Open navigation"} onClick={() => setMenuOpen(!menuOpen)} className="ml-auto grid h-11 w-11 shrink-0 place-items-center rounded-lg border border-white/20 lg:hidden">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button>
        </div>
      </div>

      <p className="sidebar-section-label hidden lg:block">Workspace</p>
      <nav id="dashboard-navigation" className={`${menuOpen ? "block" : "hidden"} px-3 pb-4 lg:block lg:px-4`} aria-label="Dashboard navigation">
        <ul className="dashboard-nav-list lg:space-y-1">
          {navItems.map(({ label, href, Icon }) => {
            const active = href === "/dashboard" ? pathname === href : pathname === href || pathname.startsWith(`${href}/`);
            return (
              <li key={label} className="dashboard-nav-item lg:shrink">
                <Link href={href} onClick={() => setMenuOpen(false)} aria-current={active ? "page" : undefined} className={`nav-link mobile-nav-link flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors focus:outline-none ${active ? "nav-link-active" : ""}`}>
      ...
      <div className={`${menuOpen ? "block" : "hidden"} border-t border-white/10 px-5 py-4 lg:hidden`}><div className="flex items-center justify-between gap-4 text-xs text-white/80"><span className="truncate">{accountLabel}</span><form action={signOut}><button className="font-semibold underline hover:text-white">Sign out</button></form></div></div>
