...
// Show fields that match the selected item type.
    ...
    <FeedbackForm action={createInventoryItem} className="card space-y-7 rounded-lg p-5 sm:p-7">
      ...
      <section>
        <div>
          ...
          <h2 className="mt-2 text-lg font-semibold">What are you adding?</h2>
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label>
            ...
            <input
              ...
              className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm"
              placeholder={
                isComputer ? "Select a room to suggest Room-PC-01" : "Dell OptiPlex 7010"
              }
            />
          </label>
          <div>
            <InputField name="assetTag" label="Asset tag" placeholder="Leave blank to generate" />
            <p className="muted mt-1 text-xs leading-5">
              Equipment receives an <code>INV-CAT-ST-ROOM-0001</code> tag and a unique QR code.
            </p>
          </div>
          <label>
          ...
          <label>
            ...
            {isSupply ? (
              <span className="muted mt-1 block text-xs">
                Supplies may use a shared stock quantity.
              </span>
            ) : (
              <span className="muted mt-1 block text-xs">
                Each equipment asset is one physical unit with its own asset tag and QR code.
              </span>
            )}
          </label>
          <label>
        ...
        <label className="mt-4 block">
          ...
          <textarea
            ...
            maxLength={5_000}
            className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm"
            placeholder="What it is used for, included accessories, or other helpful details."
          />
        </label>
      </section>

      {/* Purchase and manufacturer details. */}
      <section className="divider border-t pt-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            />
            <p className="muted mt-1 text-xs leading-5">
              Optional total paid for this inventory record. It is visible only in the record
              details and acquisition report.
            </p>
          </div>
        </div>
      </section>

      {/* Optional PC or Mac hardware profile. */}
      <section className="divider border-t pt-6">
        <label className="flex items-center gap-3 text-sm font-semibold">
          <input
            ...
            className="h-4 w-4"
          />{" "}
          This tracked asset is a PC or Mac
        </label>
        {isSupply ? (
          <p className="muted mt-2 text-sm">
            Supply records cannot include a PC or Mac hardware profile.
          </p>
        ) : null}
        {isComputer ? (
          <div className="card-muted mt-4 rounded-lg p-4">
            ...
            <p className="muted mt-1 text-xs">
              PC and Mac details stay on this record only. The room-based name suggestion remains
              editable.
            </p>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <InputField
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label>
                ...
                <textarea
                  ...
                  maxLength={5_000}
                  className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm"
                  placeholder="Chassis, display, peripherals, or other hardware configuration."
                />
              </label>
              <label>
                ...
                <textarea
                  ...
                  maxLength={5_000}
                  className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm"
                  placeholder="Operating environment, special applications, or licensing notes."
                />
              </label>
            </div>
          </div>
      ...
      <section className="divider grid gap-4 border-t pt-6 sm:grid-cols-2">
        <label>
          ...
          />
          <span className="muted mt-1 block text-xs">
            This inspection date is recorded for every item, including supplies.
          </span>
        </label>
