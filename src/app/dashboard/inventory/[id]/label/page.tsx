...
export default async function ItemLabelPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const qrDataUrl = scanUrl
    ? await QRCode.toDataURL(scanUrl, { errorCorrectionLevel: "M", margin: 1, width: 560 })
    : null;
        ...
        {scanUrl && qrDataUrl ? (
          <article className="print-label mx-auto max-w-md rounded-lg p-7 text-center">
            ...
            <p className="mt-2 text-sm text-slate-600">
              {item.assetTag ?? "No asset tag"} · {item.location.name}
            </p>
            <Image
              unoptimized
            ...
            <p className="mt-2 text-xs text-slate-600">
              Scan to view this item, request equipment, or report a problem.
            </p>
          </article>
        ) : (
          <section className="notice rounded-lg px-5 py-4 text-sm leading-6" role="alert">
            {/* Show why a public label URL is unavailable. */}
            The permanent website address needs to be configured before you can print QR labels. Ask
            your administrator to finish the website setup.
          </section>
        )}
      </div>
    </main>
