            ...
            <article className="card rounded-lg p-5 sm:p-6">
              ...
              <div className="mt-5 flex flex-col gap-5 sm:flex-row">
                ...
                <dl className="grid flex-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  {item.purchasePrice !== null ? (
                    <Detail label="Acquisition value">
              ...
              {item.itemType === ItemType.ASSET && item.quantity > 1 ? (
                <p className="notice mt-5 rounded-lg px-4 py-3 text-sm leading-6">
                  Legacy grouped asset: this record represents {item.quantity} units but has one
                  current tag and QR code. New equipment is always created as one physical unit per
                  record, so give each existing unit its own record before moving it to a different
                  room.
                </p>
              ) : null}
            </article>
                    ...
                    <div className="mt-3 space-y-3">
                      {computer.software.map((software) =>
                        canManage ? (
                          ...
                          <div
                            key={software.id}
                            className="card-muted rounded-md px-3 py-2 text-sm"
                          >
                      ...
                      {item.borrowRequests.map((request) => (
                        <li key={request.id} className="divider border-l pl-3 text-sm">
                          <p className="font-medium">
                            {request.isReservation ? "Reservation" : "Borrowing"} ·{" "}
            ...
            >
              ...
              <FeedbackForm action={updateInventoryItem} className="mt-5 space-y-4">
                ...
                {computer ? (
                  <div>
                    ...
                    <p className="muted mt-2 text-sm">
                      Tracked asset (locked while PC details exist)
                    </p>
                  ...
                  <>
                    ...
                    {item.itemType === ItemType.ASSET ? (
                      <label className="card-muted flex items-start gap-3 rounded-lg p-3 text-sm font-semibold">
                        ...
                        <span>
                          ...
                          <span className="muted mt-1 block text-xs font-normal leading-5">
                            Only PC-designated single tracked assets can have hardware and software
                            details.
                          </span>
                        </span>
                      </label>
                      ...
                      <p className="muted text-xs leading-5">
                        Supply records cannot be designated as PCs. Change the record type to a
                        tracked asset first.
                      </p>
                    )}
                  </>
                  ...
                  <label>
                    ...
                    >
                      {Object.values(ItemStatus)
                        .filter(
                          (value) =>
                            value !== ItemStatus.RETIRED || item.status === ItemStatus.RETIRED,
                        )
                        .map((value) => (
                          <option key={value} value={value}>
                ...
                <div>
                  ...
                  <p className="muted mt-1 text-xs leading-5">
                    Optional total paid for this record. It stays off the inventory list and is
                    included in the acquisition report.
                  </p>
                </div>
              ...
              <section className="divider mt-6 border-t pt-5" aria-labelledby="inspection-heading">
                ...
                <p className="muted mt-2 text-xs leading-5">
                  Last checked: {displayDate(item.lastCheckedAt)}. This records a dated inspection
                  for every item and updates the PC profile when one exists.
                </p>
                <FeedbackForm
              ...
              {item.itemType === ItemType.ASSET && item.quantity > 1 && !item.isComputer ? (
                <section
                  className="divider mt-6 border-t pt-5"
                  aria-labelledby="individualize-heading"
                >
                  ...
                  <p className="muted mt-2 text-xs leading-5">
                    This converts the current record into unit 1 and creates {item.quantity - 1} new
                    records in the same room. Every new unit gets the next compatible asset tag and
                    a unique QR code; move the units to their actual rooms afterward. This is
                    unavailable once a borrowing history exists.
                  </p>
