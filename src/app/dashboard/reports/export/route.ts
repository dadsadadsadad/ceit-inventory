...
export async function GET(request: Request) {
  ...
  if (kind === "inventory") {
    const items = await prisma.inventoryItem.findMany({ include: { category: true, location: true, computer: true }, orderBy: [{ name: "asc" }, { assetTag: "asc" }] });
  ...
  if (kind === "borrowings") {
    const requests = await prisma.borrowRequest.findMany({ include: { inventoryItem: { select: { assetTag: true, name: true } } }, orderBy: { requestedAt: "desc" } });
  ...
  if (kind === "maintenance") {
    const tickets = await prisma.maintenanceTicket.findMany({ include: { inventoryItem: { select: { assetTag: true, name: true } } }, orderBy: { openedAt: "desc" } });
