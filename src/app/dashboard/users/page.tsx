    ...
    <div className="page users-page">
      <div className="page-inner space-y-6">
        ...
        <section className="card rounded-lg p-5 sm:p-6">
          ...
          <FeedbackForm action={createUser} className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-[1.2fr_1fr_0.8fr_1fr_auto] xl:items-end">
            ...
            <label><span className="text-sm font-semibold">Username *</span><input required name="username" autoComplete="username" minLength={3} maxLength={32} pattern="[A-Za-z0-9._-]{3,32}" title="Use 3–32 letters, numbers, periods, underscores, or hyphens." className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm" placeholder="ceit.staff" /></label>
            <label><span className="text-sm font-semibold">Role *</span><select name="role" defaultValue={UserRole.STAFF} className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm">{Object.values(UserRole).map((role) => <option key={role} value={role}>{roleLabel(role)}</option>)}</select></label>
        ...
        <section className="card overflow-hidden rounded-lg">
          ...
          <div className="divide-y" style={{ borderColor: "var(--border)" }}>
            {users.map((user) => (
              <FeedbackForm key={user.id} action={updateUser} className="grid gap-4 px-5 py-5 xl:grid-cols-[minmax(12rem,1.25fr)_minmax(10rem,1fr)_10rem_8rem_minmax(12rem,1fr)_auto] xl:items-end">
                ...
                <label><span className="muted text-xs font-bold uppercase tracking-wide">Username</span><input required name="username" defaultValue={user.username} autoComplete="username" minLength={3} maxLength={32} pattern="[A-Za-z0-9._-]{3,32}" title="Use 3–32 letters, numbers, periods, underscores, or hyphens." className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm" /></label>
