...
function requiredText(formData: FormData, key: string, maximumLength = 255) {
  ...
  if (!value) throw new FormError(`${key} is required.`);
...
function idFrom(formData: FormData) {
  ...
  if (!uuidPattern.test(id)) throw new FormError("Invalid account identifier.");
  return id;
}

function emailFrom(formData: FormData) {
  const email = requiredText(formData, "email", 254).toLowerCase();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new FormError("Enter a valid email address.");
  return email;
}
...
function roleFrom(formData: FormData) {
  ...
  if (!roles.includes(role as UserRole)) throw new FormError("Choose a valid account role.");
...
export async function createUser(formData: FormData) {
  return formAction(async () => {
    ...
    if (!password) throw new FormError("A password is required for a new account.");
    ...
    try {
      ...
      if (knownWriteError(error)) throw new FormError("That email address or username is already assigned to an account.");
      throw error;
    }
...
export async function updateUser(formData: FormData) {
  return formAction(async () => {
    ...
    if (id === administrator.id && (!isActive || role !== UserRole.ADMINISTRATOR)) {
      throw new FormError("Keep your own account active and assigned as an administrator. Update another administrator first if needed.");
    }
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        await prisma.$transaction(async (transaction) => {
          ...
          if (!target) throw new FormError("This account no longer exists.");
          ...
          if (removesAdministrator) {
            ...
            if (activeAdministratorCount <= 1) throw new FormError("Keep at least one active administrator account.");
          }
