...
import { AuditAction, ItemStatus, MaintenancePriority, MaintenanceStatus, Prisma } from "@prisma/client";
...
function text(formData: FormData, key: string, maximumLength: number, required = false) {
  ...
  if (value.length > maximumLength) throw new FormError(`${key} is too long.`);
  if (required && !value) throw new FormError(`${key} is required.`);
  return value || null;
}

function id(formData: FormData, key: string) {
  const value = text(formData, key, 64, true);
  if (!value || !uuidPattern.test(value)) throw new FormError("Invalid maintenance record.");
  return value;
}
...
function enumValue<T extends string>(formData: FormData, key: string, values: readonly T[], fallback: T, required = false) {
  ...
  if (!value) {
    if (required) throw new FormError(`${key} is required.`);
    return fallback;
  }
...
function resolutionItemStatus(formData: FormData) {
  ...
  if (![ItemStatus.OK, ItemStatus.WORKING, ItemStatus.NOT_TESTED, ItemStatus.DEFECTIVE].includes(value as "OK" | "WORKING" | "NOT_TESTED" | "DEFECTIVE")) {
...
export async function createMaintenanceTicket(formData: FormData) {
  return formAction(async () => {
    ...
    await prisma.$transaction(async (transaction) => {
      ...
      if (!item || item.status === ItemStatus.RETIRED) throw new FormError("Choose an active inventory item.");
      const ticket = await transaction.maintenanceTicket.create({
        data: { inventoryItemId: itemId, title: title ?? "", description: description ?? "", priority, reportedByName: actor.username },
      });
...
export async function updateMaintenanceTicket(formData: FormData) {
  return formAction(async () => {
    ...
    const priority = enumValue(formData, "priority", Object.values(MaintenancePriority), MaintenancePriority.NORMAL, true);
    ...
    const ticket = await prisma.$transaction(async (transaction) => {
      ...
      if (!existing) throw new FormError("This maintenance request no longer exists.");
      const submittedVersion = String(formData.get("updatedAt") ?? "");
      if (submittedVersion && submittedVersion !== existing.updatedAt.toISOString()) throw new FormError("This maintenance request changed since you opened it. Refresh the page before saving again.");
      if (itemStatus && existing.inventoryItem.status === ItemStatus.RETIRED) throw new FormError("Restore this item from Inventory before changing its status.");
      if (itemStatus === ItemStatus.OK || itemStatus === ItemStatus.WORKING) {
        const outstanding = await transaction.borrowRequest.count({ where: { inventoryItemId: existing.inventoryItemId, status: { in: ["BORROWED", "RETURN_REQUESTED"] } } });
        if (outstanding) throw new FormError("Confirm the equipment’s return before making it available again.");
      }
        ...
        data: auditEventData({
          ...
          metadata: {
            changes: {
              ...
              priority: existing.priority === priority ? undefined : { from: existing.priority, to: priority },
