...
function ticketTone(status: MaintenanceStatus) {
  return status === MaintenanceStatus.RESOLVED ? "status-pill status-pill-positive" : "status-pill status-pill-pending";
}
  ...
  return (
    <div className="page maintenance-page">
      <div className="page-inner space-y-6">
        ...
        <FeedbackForm action={createMaintenanceTicket} className="card space-y-4 rounded-lg p-5 sm:p-6">
          <div>
            <h2 className="text-lg font-semibold">Report an item issue</h2>
            <p className="muted mt-1 text-sm">Use this when an item needs inspection, repair, or replacement.</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <label>
              <span className="text-sm font-semibold">Inventory item *</span>
              <select required name="itemId" defaultValue={items.some((item) => item.id === selectedItem) ? selectedItem : ""} className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm">
                <option value="" disabled>Select an item</option>
                {items.map((item) => <option key={item.id} value={item.id}>{item.name}{item.assetTag ? ` · ${item.assetTag}` : ""}</option>)}
              </select>
            </label>
            <label>
              <span className="text-sm font-semibold">Priority</span>
              <select name="priority" defaultValue={MaintenancePriority.NORMAL} className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm">
                {Object.values(MaintenancePriority).map((priority) => <option key={priority} value={priority}>{priorityLabel(priority)}</option>)}
              </select>
            </label>
          </div>
          <label className="block">
            <span className="text-sm font-semibold">Issue title *</span>
            <input required name="title" maxLength={255} className="field mt-2 w-full rounded-lg px-3 py-2.5 text-sm" placeholder="e.g. Screen does not power on" />
          </label>
          <label className="block">
        ...
        <form className="card flex flex-wrap items-end gap-3 rounded-lg p-4">
          <label>
            ...
            <select name="status" defaultValue={status ?? ""} className="field mt-2 rounded-lg px-3 py-2.5 text-sm">
              ...
              {Object.values(MaintenanceStatus).map((value) => <option key={value} value={value}>{statusLabel(value)}</option>)}
            </select>
          </label>
          <button className="primary-button rounded-lg px-4 py-2.5 text-sm font-semibold">Filter</button>
          <Link href="/dashboard/maintenance" className="card card-link rounded-lg px-4 py-2.5 text-sm font-semibold">Clear</Link>
        </form>
