...
describe("inventory label URLs", () => {
  ...
  it("does not use a Vercel or arbitrary public request host when no canonical URL is configured", () => {
    ...
    expect(
      inventoryLabelAppUrl("not a URL", new Headers({ host: "inventory.example.edu" })),
    ).toBeNull();
  });
  ...
  it("uses Vercel's permanent production domain instead of a preview request host", () => {
    expect(
      inventoryLabelAppUrl(
        undefined,
        new Headers({ host: "ceit-inventory-git-main-team.vercel.app" }),
        "inventory.example.edu",
      ),
    ).toBe("https://inventory.example.edu");
    expect(inventoryLabelAppOrigin(undefined, "inventory.example.edu")).toBe(
  ...
  it("provides a guarded configured origin for the in-app scanner", () => {
    expect(inventoryLabelAppOrigin("https://inventory.example.edu/ceit")).toBe(
      "https://inventory.example.edu",
    );
